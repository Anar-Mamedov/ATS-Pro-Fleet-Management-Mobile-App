...
export default function ServiceDetailScreen() {
  ...
  const { siraNo, plaka, servisTanimi } = useLocalSearchParams<{ siraNo: string; plaka: string; servisTanimi: string }>();
  ...
  return (
    <SafeAreaView edges={['top', 'left', 'right']} style={{ flex: 1, backgroundColor: isDark ? '#000000' : '#F2F2F7' }}>
      ...
      <YStack flex={1} backgroundColor="$background">
        {/* Header */}
        <LinearGradient colors={isDark ? ['#0A84FF', '#0055CC'] : ['#0A84FF', '#3DA5FF']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.header}>
          <Pressable onPress={() => router.back()} style={styles.backButton} hitSlop={12}>
            <Ionicons name="arrow-back" size={22} color="#FFFFFF" />
          </Pressable>
          <YStack flex={1} gap={2}>
            <Text fontSize={17} fontWeight="700" color="white" numberOfLines={1}>
              {servisTanimi || t('serviceOperations')}
            </Text>
        ...
        <YStack flex={1}>
          {activeTab === 'documents' ? (
            <DocumentUpload refId={refId} refGroup="Servis_islemleri" editable />
          ) : (
            <ResimUpload refId={refId} refGroup="Servis_islemleri" />
          )}
        </YStack>
      </YStack>
    </SafeAreaView>
